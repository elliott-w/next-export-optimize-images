...
const RemoteImage = forwardRef<HTMLImageElement, ImageProps>(({ src, ...props }, forwardedRef) => {
  ...
  // Explicit width prop wins (consumer told us exactly what the remote source is);
  // otherwise the build-emitted intrinsic-map covers URLs declared in
  // config.remoteImages. If both miss, the full ladder gets enrolled.
  ...
  if (intrinsic !== undefined && props.loader === undefined) {
    return <Image {...props} src={src} loader={imageLoader({ intrinsic })} ref={forwardedRef} />
  }
